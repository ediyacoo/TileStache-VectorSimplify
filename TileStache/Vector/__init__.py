...
Common parameters:
  ...
  simplify:
	Default is None.
	Unit is degree.
	Set the simlify tolerance to simplify geometries.
...
try:
    from json import JSONEncoder, loads as json_loads, encoder
except ImportError:
    from simplejson import JSONEncoder, loads as json_loads, encoder
...
def _get_features(coord, properties, projection, layer, clipped, projected, spacing, id_property, simplify, decimalNumber):
    ...
    for feature in layer:
		...
		#customization=====================================================================================
		#simplify geometry
        if(simplify is not None and float(simplify) > 0 and geometry):
				#from http://www.itopen.it/2012/01/11/tilestache-vector-simplify-provider/==================
				...
				#============================================================================================
				tolerance = float(simplify) * ((abs(xspan) + abs(yspan)) / 2)
				#print "tolerance=" + str(tolerance)
				geometry=geometry.SimplifyPreserveTopology(tolerance)
        
		#minize coodintaes
        encoder.FLOAT_REPR = lambda o: format(o, '.'+str(decimalNumber)+'f')
		#customization=====================================================================================
...
class Provider:
    ...
    def __init__(self, layer, driver, parameters, clipped, verbose, projected, spacing, properties, precision, id_property, simplify, decimalNumber):
        self.layer      = layer
        self.driver     = driver
    ...
    def prepareKeywordArgs(config_dict):
        ...
        kwargs['simplify'] = float(config_dict.get('simplify', None))
        kwargs['decimalNumber'] = int(config_dict.get('decimalNumber', 4))
    ...
    def renderTile(self, width, height, srs, coord):
        ...
        features = _get_features(coord, self.properties, self.layer.projection, layer, self.clipped, self.projected, self.spacing, self.id_property, self.simplify, self.decimalNumber)
